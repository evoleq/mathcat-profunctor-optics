...
import org.evoleq.math.cat.suspend.profunctor.transformer.CoMonoidal

interface CoMonoidalLight<A, B, S, T> : CoMonoidal<S, T> {
    
    suspend fun <U, V> branch(coMonoidalLight: CoMonoidalLight<A, B, U, V>): CoMonoidalLight<A,B,Either<S, U>, Either<T, V>>
    
    override suspend fun <U, V> branch(comonoidal: CoMonoidal<U, V>): CoMonoidalLight<A, B,Either<S, U>, Either<T, V>> {
