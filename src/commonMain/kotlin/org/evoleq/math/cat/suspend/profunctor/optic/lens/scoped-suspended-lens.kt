...
import org.evoleq.math.cat.suspend.lens.getter
import org.evoleq.math.cat.suspend.lens.setter
...
import org.evoleq.math.cat.suspend.profunctor.Cartesian
...
data class Lens<A, B, S,  T>(private val lens: suspend CoroutineScope.(Cartesian<A, B>)-> Cartesian<S, T>) : Optic<A, B, S, T> by Optic(lens as suspend CoroutineScope.(Profunctor<A, B>)-> Cartesian<S, T>)
...
@MathCatDsl
@Suppress("FunctionName")
suspend fun <A, B, S, T>  ILens(lens: Lens<A, B, S, T>): ILens<S, T, A, B> = coroutineScope {
    by(lens)(Cartesian(ILens<A, B, A, B>({ a: A -> a }, { p: Pair<A, B> -> p.second }))) /*as CartesianILens<A, B, S, T>*/ as ILens<S, T, A, B>
}
